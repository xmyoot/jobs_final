...
const getPageJobs = async () => {
    try {
        let companyLogo, allJobs = [], start = 1, resultCount = 2, resultTracker = 0
        ...
        while( resultCount > 0 ) {
            ...
            const jobObjects = jobs.map((i, e)=> {
                const job = $(e)
                const position = job.find('[data-test-id="svx-job-title"]').text().trim() || ''
                const company = job.find('[data-test-id="svx-job-company"]').text().trim() || ''
                const location = job.find('[data-test-id="svx-job-location"]').text().trim() || ''
                const salary = job.find('[data-test-id="svx-job-salary-range"]').text().trim() || ''
                const jobUrl = job.attr('href')
                const logoString = job.find('[data-test-id="svx-job-logo"]').attr('src') || ''
